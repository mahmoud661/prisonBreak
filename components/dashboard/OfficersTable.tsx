...
export const OfficersTable: React.FC<OfficersTableProps> = ({ officers }) => {
  ...
  const [searchTerm, setSearchTerm] = useState<string>("");  const [filteredOfficers, setFilteredOfficers] = useState<Officer[]>(officers);
  ...
  useEffect(() => {
    ...
    const filtered = officers.filter((officer) => {
      // Apply shift filter
      const matchesShift =
        shiftFilter === "all" || officer.shift.toLowerCase() === shiftFilter;

      // Apply role filter
      const matchesRole =
        roleFilter === "all" || officer.role.toLowerCase() === roleFilter;
  ...
  const handleRowClick = (officer: Officer) => {
    // Set the selected officer and open the info modal
    setSelectedOfficer(officer);
    infoModalRef.current?.showModal(officer);
  };
  return (
    <>
          ...
          <table className="w-full">
            ...
            <tbody className="text-sm">
              {filteredOfficers.map((officer, index) => (
                <tr
                  key={index}
                  className="border-b border-zinc-200/70 dark:border-zinc-700/50 last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-700/50"
      ...
      {/* Info Modal for displaying officer details */}
      <InfoModal ref={infoModalRef} title="Officer Details" />
    </>
  );
};
